...
export default class Amazon extends ScrapeCommand<typeof Amazon> {
  ...
  static flags = {
    ...
    yearFilter: Flags.integer({
      aliases: [`yearFilter`],
      description: `Filters a year`,
      env: `AMAZON_YEAR_FILTER`,
    }),
    pageFilter: Flags.integer({
      aliases: [`pageFilter`],
      description: `Filters a page`,
      env: `AMAZON_PAGE_FILTER`,
    }),
  };
  ...
  private async goToOrderPage(amazon: AmazonDefinition) {
    ...
    await this.currentPage.goto(amazon.orderPage, {
  ...
  private async processOrderPage(
    ...
    const nextPageUrl = this.checkForLastPage(
      orderPage,
      orderPageCount,
      currentYear,
      amazon,
  ...
  private async getOrderPageCount(currentYear: number) {
    ...
    await this.currentPage.select(
      amazonSelectors.yearFilter,
      `year-${currentYear}`,
    );
    ...
    try {
      ...
      orderPageCount = 1;
      this.logger.error(
        `Couldn't get orderPageCount ${orderPageCount} within ${this.selectorWaitTimeout}ms. Assume only one page.`,
      );
    }
  ...
  private checkForLastPage(
    ...
    if (orderPage + 1 != orderPageCount) {
      ...
      if (this.flags.pageFilter != orderPage + 1) {
        this.logger.info(
          `Page "${orderPage + 1}" done. Skipping to next page.`,
        );
      ...
      this.logger.info(
        `Last page "${orderPageCount}" reached. Going to next year.`,
      );
      return null;
    }
  }

  private async clickInvoiceSpan(
    orderCard: ElementHandle<Element>,
    orderIndex: number,
  ) {
    const amazonSelectors = this.selectors;
  ...
  private getInvoices(invoiceUrls: string[], orderIndex: number): Invoice[] {
    if (invoiceUrls.length == 0) {
      this.logger.warn(
        `No invoices found. Order may be undelivered. Check again later.`,
      );
      return [];
    } else {
      const invoices: Array<Invoice> = invoiceUrls.map(
        (invoiceUrl) =>
          ({ url: invoiceUrl, status: InvoiceStatus.determined }) as Invoice,
      );
  ...
  private async getInvoiceDocumentsFromOrder(order: Scrape) {
    for (const [invoiceIndex, invoice] of order.invoices.entries()) {
      ...
      try {
        ...
        this.logger.error(
          `Error while fetching from url "${invoiceUrl}": ${ex.message}`,
        );
      }
      const fileBuffer = this.getFileBuffer(
        fileReaderString,
        invoice,
        order,
        invoiceUrl,
      ...
      if (fileBuffer) {
        ...
        this.logger.info(
          `Checking if folder exists. If not, create: ${this.flags.fileDestinationFolder}`,
        );

        const { destPluginFileFolder, pathNormalized } = this.getPaths(
          invoiceUrl,
          order,
          invoiceIndex,
        );
    ...
    const possibleYears = yearFilter
      ...
      : await (
          await this.currentPage.$$eval(
            `${this.selectors.yearFilter} option`,
            (handles: Array<HTMLOptionElement>) =>
              handles.map((option) => parseInt(option.innerText)),
          )
        ).filter((n) => n);
  ...
  private async getInvoiceUrls(orderIndex: number): Promise<string[]> {
    ...
    const popoverSelectorResolved = amazonSelectors.popover.replace(
      `{{index}}`,
      (orderIndex + 1).toString(),
    );
    try {
      const popover = await this.currentPage.waitForSelector(
        popoverSelectorResolved,
        { timeout: selectorWaitTimeout },
      );
      this.logger.debug(`Got popover ${orderIndex + 1} -> ${popover}`);
      invoiceList = await popover.waitForSelector(amazonSelectors.invoiceList, {
        timeout: selectorWaitTimeout,
      });
      invoiceUrls = await invoiceList.$$eval(
        amazonSelectors.invoiceLinks,
        (handles: Array<HTMLAnchorElement>) => handles.map((a) => a.href),
      );
      this.logger.debug(`Got invoiceUrls ${orderIndex + 1} -> ${invoiceUrls}`);
    } catch (ex) {
      this.logger.error(
        `Couldn't get popover ${popoverSelectorResolved} within ${selectorWaitTimeout}ms. Skipping`,
      );
    }
    return invoiceUrls;
  }
  ...
  private async getOrder(orderCard: ElementHandle<Element>) {
    const amazonSelectors = this.selectors;
    const amazon = this.definition;
    const orderNumber: string = await orderCard.$eval(
      amazonSelectors.orderNr,
      (handle: HTMLElement) => handle.innerText.trim(),
    );

    const orderDate = await orderCard.$eval(
      amazonSelectors.orderDate,
      (handle: HTMLElement) => handle.innerText,
    );
    const orderDateLuxon = DateTime.fromFormat(orderDate, `DDD`, {
      locale: amazon.lang,
    }).toISODate();
    const order: Scrape = {
      date: orderDateLuxon,
  ...
  private async endProcess(
    ...
    const invoiceCount = orders.reduce(
      (prev, order) => prev + order.invoices.length,
      0,
    );
    this.logger.info(
  ...
  private getSelectors(tld: string): {
    amazonSelectors: AmazonSelectors;
    amazon: AmazonDefinition;
  } {
    ...
    const amazonSelectors: AmazonSelectors = {
      ...
      captchaImage: `.cvf-captcha-img img[alt="captcha"]`,
    };
    return { amazonSelectors, amazon };
  }

  private writeFile(
    destPluginFileFolder: string,
    ...
    if (!fs.existsSync(pathNormalized)) {
      ...
      this.logger.warn(
        `Invoice "${path.basename(pathNormalized)}" of order "${
          order.number
  ...
  private getPaths(invoiceUrl: string, order: Scrape, invoiceIndex: number) {
    const destPluginFileFolder = path.resolve(
      path.join(
        this.flags.fileDestinationFolder,
        this.flags.subFolderForPages ? this.pluginName : ``,
        `/`,
      ),
      `./`,
    );
    const fileExtention =
      path.extname(invoiceUrl).split(`?`)[0] ??
      this.flags.fileFallbackExentension;
  ...
  private getFileBuffer(
    ...
    if (!fileReaderString) {
      this.logger.error(
        `FileReaderString is empty returned from page for order "${order.number}" and url "${invoiceUrl}"`,
      );
      return null;
    }
    ...
    if (!fileBuffer) {
      this.logger.error(
        `Failed to create buffer from fileReader for order "${order.number}"`,
      );
      return null;
    }
    invoice.status = InvoiceStatus.downloaded;
  ...
  private async getFileReaderString(
    pdfPage,
    invoiceUrl: string,
  ): Promise<string> {
