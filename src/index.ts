...
(async () => {
    ...
    program
        ...
        .option(`-dest, --file-destination-folder`, `Destination folder where the downloaded files are stored`, config.fileDestinationFolder)
        ...
        .requiredOption(`-u, --amazon-username`, `Amazon username to use.`, config.amazonUsername)
        .requiredOption(`-p, --amazon-password`, `Amazon password to use.`, config.amazonPassword)
        .parse();
    ...
    const logger = winston.createLogger({
        ...
        transports: [
            ...
            new winston.transports.File({ filename: path.join(options.logPath, `verbose.log`).normalize(), level: `verbose` }),
    ...
    // logger.debug(`Env: ${JSON.stringify(config, null, 4)}`);

    if (!options.amazonUsername && !options.amazonPassword) {
    ...
    logger.verbose(`Page language: ${amazon.lang}`);
    ...
    logger.verbose(`First possible year: ${firstPossibleYear}`);
    logger.verbose(`Last possible year: ${lastPossibleYear}`);
        ...
        for (const orderPage of [...Array(orderPageCount).keys()]) {
            ...
            for (const [orderIndex, orderCard] of orderCards.entries()) {
                ...
                if (invoiceUrls.length == 0) {
                    logger.warn(`No invoices found. Order may be undelivered. Check again later.`);
                } else {
                    ...
                    logger.info(`${invoices.length} invoices found 📃`);
                    logger.debug(`Got invoiceUrls ${(orderIndex + 1)} -> ${invoiceUrls}`);
                }
                orders.push(order);
            ...
            if ((orderPage + 1) != orderPageCount) {
                ...
                logger.debug(`Nextpage url: ${nextPageUrl}`);
    ...
    logger.info(`Processing done. Processed ${invoiceCount} invoices in ${timeElapsed.minutes.toFixed(2)} minutes.`);
